...
| **Vũ Đức Duy** | **Member C:** Dashboards, Blueprint & Demo | Triển khai Dashboard ảo hóa với 6 panels (lấy metrics từ hệ thống). Setup file `blueprint-template.md`. Thu thập log/ảnh làm chứng cứ (evidence) & Dẫn dắt buổi Demo. | Cấu hình Dashboard, `blueprint-template.md`, `grading-...` |
...
| **Nhữ Gia Bách** | **Member E:** SLO & Alerts | Thiết lập `slo.yaml` và `alert_rules.yaml`. Xác định ngưỡng vi phạm (VD: P95 latency hoặc Error Rate). Viết kịch bản xử lý cảnh báo trong `alerts.md`. | `slo.yaml`, `alert_rules.yaml`, `alerts.md` |
...
### Sprint 2: Đo lường & Cảnh báo (Bách & Duy)
- **Bách:** Dựa vào mô hình trace ở Sprint 1, tiến hành định nghĩa file YAML `slo.yaml`, `alert_rules.yaml` (các mốc time-out, error 500).
- **Duy:** Dựa vào file `docs/dashboard-spec.md`, cài cắm công cụ hiển thị đồ thị / chart lấy log metric của nhóm thành 6 blocks đúng chuẩn.
...
3. Cản báo (Alerts) của Bách có bị trigger không?
4. Đồ thị Dashboard của Duy có gồ lên bất thường không?
